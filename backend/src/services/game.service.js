...
import { GAME_RESULTS } from '../config/constants.js';
import { sequelize } from '../config/db.js';
import { gameRoundsTotal, gameChoicesTotal } from '../utils/metrics.js';

export class GameService {
  static async submitGameResult(gameData) {
    const { username, result, player_choice, computer_choice, session_duration } = gameData;

    // Track metrics for choices
    if (player_choice) {
      gameChoicesTotal.inc({ choice: player_choice });
    }
    if (computer_choice) {
      gameChoicesTotal.inc({ choice: computer_choice });
    }
    ...
    // Update user stats and track game result metric
    switch (result) {
      case GAME_RESULTS.WIN:
        await user.increment('wins');
        gameRoundsTotal.inc({ result: 'win' });
        break;
      case GAME_RESULTS.LOSE:
        await user.increment('losses');
        gameRoundsTotal.inc({ result: 'lose' });
        break;
      case GAME_RESULTS.DRAW:
        await user.increment('draws');
        gameRoundsTotal.inc({ result: 'draw' });
        break;
    }
